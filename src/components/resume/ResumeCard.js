...
const ResumeCard = ({title, subTitle, des}) => {
  return (
    <div className='w-full h-1/2 group flex'>
        ...
        <div id='resumecard' className='w-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300 rouned-lg p-4 
                            lgl:px-10 flex flex-col  justify-center font-titleFont '>
            <div className='flex flex-col '>
                <h3 className='text-xl md:text-2xl text-gray-300 group-hover:text-white duration-300'> {title}</h3>
                <p className='text-sm mb-10  text-gray-400 group-hover:text-white duration-300'>{subTitle}</p>
              <div>
